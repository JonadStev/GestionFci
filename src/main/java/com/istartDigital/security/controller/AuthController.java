...
public class AuthController {
    ...
    public Map<String, String> nuevo(@RequestBody NuevoUsuario nuevoUsuario){
        ...
        Usuario usuario = new Usuario(
                ...
                nuevoUsuario.getCarrera()
        );
        Set<Rol> roles = new HashSet<>();
        if(nuevoUsuario.getRoles().contains("gestor")){
            roles.add(rolService.getByRolNombre(RolNombre.ROLE_GESTOR).get());
            roles.add(rolService.getByRolNombre(RolNombre.ROLE_DIRECTOR).get());
            roles.add(rolService.getByRolNombre(RolNombre.ROLE_INVESTIGADOR).get());
        }else if (nuevoUsuario.getRoles().contains("director")){
            roles.add(rolService.getByRolNombre(RolNombre.ROLE_DIRECTOR).get());
        }else {
            roles.add(rolService.getByRolNombre(RolNombre.ROLE_INVESTIGADOR).get());
        }
